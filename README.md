...
## 实现原理
1读取当前歌曲信息 — 从 localStorage['current_song'] 获取歌曲的 url、title、artist 等字段

2在播放栏添加下载按钮 — 注入到 .extra-controls（与其他控制按钮同级），使用 Font Awesome 的 fa-download 图标

3流式下载 — 通过 fetch + ReadableStream 分块读取音频数据，显示进度百分比

4触发浏览器下载 — 将下载的音频数据转为 Blob URL，通过``` <a download> ```触发下载
